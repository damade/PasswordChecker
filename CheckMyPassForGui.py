...
def main(Passwords):
    totalPassword = list(map(str, Passwords.split(",")))
    output = []
    for password in totalPassword:
        count = pwned_api_check(password.strip())
        if count:
            output += [f" \"{password.strip()}\" was found {count} times....."]
            output += [" oops, You would need to change your password to secure it\n"]
            output += [" "]
        else:
            output += [f" \"{password.strip()}\" was not found....."]
            output += [" Your password is secure, Carry on !!!!\n"]
            output += [" "]
    output += []
    output += [' done!']
    return output
